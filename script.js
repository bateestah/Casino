...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const spinRoulette = () => {
    ...
    const pocketAngle = -90 + targetIndex * segmentAngle;
    const desiredRotation =
      ((pointerAngle - pocketAngle) % 360 + 360) % 360;
    let rotationDelta = desiredRotation - currentNormalized;
    rotationDelta = ((rotationDelta % 360) + 360) % 360;
    const extraSpins = 4 + Math.floor(Math.random() * 3);
    const finalRotation =
      currentRotation + rotationDelta + extraSpins * 360;
